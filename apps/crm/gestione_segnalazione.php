...
<body>
                ...
                <?php else: ?>
                    <?php foreach ($segnalazioni as $s): ?>
                        <div class="report-record"
                             id="report-record-<?php echo $s['id_segnalazione']; ?>"
                             data-title="<?php echo strtolower(htmlspecialchars($s['titolo'], ENT_QUOTES)); ?>"
                             data-desc="<?php echo strtolower(htmlspecialchars($s['descrizione'], ENT_QUOTES)); ?>">
                                ...
                                <div class="details-grid">
                                    ...
                                    <div class="detail-box">
                                        ...
                                        <form class="management-form">
                                            ...
                                            <div class="form-group">
                                                <label for="note_interne-<?php echo $s['id_segnalazione']; ?>">Note Interne:</label>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Apri/chiudi fisarmonica e gestisci update
        container.addEventListener('click', function(event) {
            const summary = event.target.closest('.report-summary');
            if (summary) {
                summary.closest('.report-record').classList.toggle('is-open');
            }
            ...
            if (updateButton) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const statusBadge = recordDiv.querySelector('.status-badge');
        ...
        const records = document.querySelectorAll('.report-record');
        input.addEventListener('input', () => {
            const q = input.value.trim().toLowerCase();
            records.forEach(r => {
                const title = r.dataset.title;
                const desc  = r.dataset.desc;
                r.style.display = (!q || title.includes(q) || desc.includes(q)) ? '' : 'none';
            });
        });
    });
    </script>
